...
const STABLETRUST_CONTRACT_ADDRESS = '0x29E4fd434758b1677c10854Fa81C2fc496D76E62'
...
async function main () {
  console.log('Starting Tether x Fairblock Confidential Demo (WDK Protocol Integration)...')
  try {
    ...
    const senderProtocol = new ConfidentialProtocolEvm(sender, CONFIDENTIAL_CONFIG)
    const receiverProtocol = new ConfidentialProtocolEvm(receiver, CONFIDENTIAL_CONFIG)
    ...
    let senderPublicBalanceBefore = await senderProtocol.getPublicBalance({
      token: USDT0_CONTRACT_ADDRESS
    })
    
    console.log(`Pre-Deposit Confidential Balance(Sender): ${ethers.formatUnits(senderConfBalanceBefore.amount, 2)} USDT0`)
    console.log(`Pre-Deposit Public Balance(Sender): ${ethers.formatUnits(senderPublicBalanceBefore, 6)} USDT0`)
    ...
    let senderPublicBalanceAfter = await senderProtocol.getPublicBalance({
      token: USDT0_CONTRACT_ADDRESS
    })
    
    console.log(`Post-Deposit Confidential Balance(Sender): ${ethers.formatUnits(senderConfBalanceAfter.amount, 2)} USDT0`)
    console.log(`Post-Deposit Public Balance(Sender): ${ethers.formatUnits(senderPublicBalanceAfter, 6)} USDT0`)
    ...
    console.log('Status: Confidential Transfer is completed. Transfer amount is hidden on-chain.')
    console.log(`Tx Hash: ${txRes.hash}`)
    console.log(`View Transaction: ${EXPLORER_URL}${txRes.hash}`)

    let senderConfBalanceAfterTransfer = await senderProtocol.getConfidentialBalance({
      token: USDT0_CONTRACT_ADDRESS
    })
    console.log(`Post-Transfer Confidential Balance(Sender): ${ethers.formatUnits(senderConfBalanceAfterTransfer.amount, 2)} USDT0`)
    ...
    let recipientPublicBalanceBefore = await receiverProtocol.getPublicBalance({
      token: USDT0_CONTRACT_ADDRESS
    })
    console.log(`Pre-Withdraw Public Balance(Recipient): ${ethers.formatUnits(recipientPublicBalanceBefore, 6)} USDT0`)
    ...
    let recipientPublicBalanceAfter = await receiverProtocol.getPublicBalance({
      token: USDT0_CONTRACT_ADDRESS
    })
    console.log(`Post-Withdraw Public Balance(Recipient): ${ethers.formatUnits(recipientPublicBalanceAfter, 6)} USDT0`)
